    ...
    <div class="card-body">
        ...
        <div class="card-text d-flex gap-2 align-items-center flex-wrap">
            @if ($product->discount > 0)
                <div class="text-black fw-medium">{{ $product->getDiscountPrice() }}đ</div>
                <div class="d-flex gap-2 align-items-center">
                    <div class="text-secondary text-decoration-line-through fw-medium">{{ $product->price }}đ
                    </div>
                    <div class="text-danger fw-medium">-{{ $product->discount }}%</div>
                </div>
            @else
                <div class="text-black fw-medium">{{ $product->price }}đ</div>
            @endif
        </div>
    </div>
</a>